...
class PlayerView: UIView {
    ...
    override func observeValue(forKeyPath _: String?, of _: Any?, change _: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
    ...
    public func prepareToPlay() {
        ...
        if let player = player {
            PlayerManager.shared.remove(owner: viewController() ?? self, player: player)
        }
    ...
    public func play() {
        PlayerManager.shared.play(owner: viewController() ?? self, player: player)
    }
    ...
    private func addPlayerItemEndObserver() {
        ...
        playToEndObserverToken = NotificationCenter.default.addObserver(forName: .AVPlayerItemDidPlayToEndTime, object: playerItem, queue: OperationQueue.main) { [weak self] _ in
    ...
    @objc private func tapGestureEvent(gesture _: UITapGestureRecognizer) {
        // 当前暂停状态
        if player.rate == 0 {
            play()
        ...
        delegate?.onPlayerStatusChange(status: viewModel?.status ?? PlayerStatus.none)
    }
}
